...
// ...
{
	...
	/*
	if( $javo_getVisibleLocationType == "gg_ac" && false )
	{
		$javo_location->term		= " hidden";
	}else{
		$javo_location->gg_ac		= " hidden";
	}*/
}
...
{
	$javo_integer	= 0;
	$javo_featureds = $wpdb->get_results( $wpdb->prepare("select p.ID from $wpdb->posts as p inner join $wpdb->postmeta as m on p.ID = m.post_id where 1=1 and m.meta_key=%s and	m.meta_value=%s and p.post_status=%s", 'javo_this_featured_item', 'use', 'publish'), OBJECT );
}
		...
		<div class="pull-left">
			<div class="javo-map-tab-panel-container">
				<div class="javo-map-tab-panel-toggle btn">
					<span><?php _e( "Search Options",'javo_fr');?></span>
					...
					<div class="javo-map-tab-panel hidden">
						...
						<form role="form">
							...
							<div class="form-group">
								...
								<div>
									<input type="text" class="form-control" value="<?php echo $javo_current_key;?>" placeholder="<?php _e("Keyword", 'javo_fr');?>" id="jvtb_keyword" data-javo-keyword>
								</div>
							</div><!-- /.form-group -->

							<!-- Rating --->
							<div class="form-group javo-map-tab-panel-bedroom">
								<div class="row">
									<div class="col-md-12">
							...
							<!-- BethRoom --->
							<div class="form-group javo-map-tab-panel-pet">
								<div class="row">
				...
				<div class="javo-map-tab-lists-header clearfix">
					...
					<div class="pull-right">
						<div class="inline-block vertical-bottom">
							...
							<!--<select name="javo_map_tab_order_by" class="form-control">
								<option value="date"><?php _e("Date", 'javo_fr');?></option>
								<option value="name"><?php _e("Name", 'javo_fr');?>
								<option value="rating"><?php _e("Rating", 'javo_fr');?></option>
							</select>-->
						</div>
						<div class="inline-block vertical-bottom">

							<!--<div><?php _e("Order", 'javo_fr'); ?></div>-->
			...
			<div class="col-md-12">
				<div class="btn-group btn-group-justified pull-right">
					<a class="btn btn-primary btn-sm" onclick="window.javo_map_tab_func.brief_run(this);" data-id="{post_id}">
						<i class="fa fa-user"></i> <?php _e("Summary", "javo_fr"); ?>
					</a>
					<a href="{permalink}" class="btn btn-primary btn-sm">
						<i class="fa fa-group"></i> <?php _e("Details", "javo_fr"); ?>
					</a>
...
jQuery( function( $ ){
	...
	window.javo_map_tab_func = {
		...
		init: function(){
			...
			//this.items			= JSON.parse( $('[javo-map-all-items]').val() );
		...
		, resize: function()
		{
			...
			if( $( "div#wpadminbar" ).length > 0 ){
				//map_offsetY -= parseInt( $( "div#wpadminbar" ).outerHeight() );
			}
